...
> * 英文与非标点的中文之间需要有一个空格，如”使用LeanCloud开发移动应用“而不是”使用LeanCloud开发移动应用“。
...
> * 如果是全篇转载，请在全文开头显着位置注明出处并链接至原文，如：

本文转载自[维基语录]（https://en.wikiquote.org/wiki/Alan_Perlis）
...
> * 字体和字号的一致：在富文本格式文档中，特别是HTML邮件中，常有人因为从不同来源复制粘贴而导致同一层次的文本字体和字号不一致。这给人不专业的感觉，请避免。
> * App是应用程序的缩写，发音为/æp/，所以要注意不要把三个字母拆开念.App是一个普通名词而不是多个单词的首字母缩写，所以不应该用全大写的APP。和其他词一样，大小写规则取决于是否处于句首，标题，或特殊短语（如App Store）中。大多数情况下应该使用中文「应用」以避免这样的问题。
...
1.有研究显示，打字的时候不喜欢在中文和英文之间加空格的人，感情路都走得很辛苦，有七成的比例会在34岁的时候跟自己不爱的人结婚，而其余三成的人最后只能把遗产留给自己的猫。毕竟爱情跟书写都需要适时地留白。
